...
<section class="min-h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 py-12 px-6">
    <div class="mx-auto max-w-4xl space-y-8 rounded-3xl border border-white/10 bg-white/5 p-8 text-slate-100 shadow-2xl shadow-slate-950/40 backdrop-blur-xl">
        <header class="space-y-2">
            ...
            <p class="text-sm text-slate-400">Upload product imagery and fill in key specifications.</p>
        </header>
        ...
        <form method="POST" action="{{ route('admin.products.store') }}" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid gap-6 md:grid-cols-2">
                ...
                <div>
                    ...
                    <input id="brand" name="brand" value="{{ old('brand') }}" required
                        class="w-full rounded-xl border border-white/10 bg-slate-900/80 px-4 py-3 text-slate-100 shadow-sm placeholder:text-slate-500 focus:border-cyan-400 focus:outline-none" />
                </div>
                ...
                <div>
                    ...
                    <input id="stock" name="stock" value="{{ old('stock', 0) }}" type="number" min="0" required
                        class="w-full rounded-xl border border-white/10 bg-slate-900/80 px-4 py-3 text-slate-100 shadow-sm placeholder:text-slate-500 focus:border-cyan-400 focus:outline-none" />
                </div>
                ...
                <div>
                    ...
                    <select id="category_id" name="category_id" required
                        class="w-full rounded-xl border border-white/10 bg-slate-900/80 px-4 py-3 text-slate-100 shadow-sm focus:border-cyan-400 focus:outline-none">
                ...
                <div>
                    <label for="image" class="mb-1 block text-sm font-medium text-slate-200">Product image</label>
                    ...
                    <p class="mt-2 text-xs text-slate-500">Upload high-resolution JPG or PNG (max 4MB).</p>
                </div>
            </div>
            ...
            <div class="grid gap-6 md:grid-cols-2">
                ...
                <div>
                    ...
                    <input id="ports" name="ports" value="{{ old('ports') }}"
                        class="w-full rounded-xl border border-white/10 bg-slate-900/80 px-4 py-3 text-slate-100 shadow-sm placeholder:text-slate-500 focus:border-cyan-400 focus:outline-none" />
                </div>
                <div class="md:col-span-2">
                    <label for="operating_system" class="mb-1 block text-sm font-medium text-slate-200">Operating system</label>
                    <input id="operating_system" name="operating_system" value="{{ old('operating_system') }}"
                        class="w-full rounded-xl border border-white/10 bg-slate-900/80 px-4 py-3 text-slate-100 shadow-sm placeholder:text-slate-500 focus:border-cyan-400 focus:outline-none" />
                </div>
            </div>
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-end gap-3">
                ...
                <button type="submit" class="inline-flex items-center justify-center rounded-xl bg-gradient-to-r from-cyan-400 to-indigo-500 px-6 py-2.5 text-sm font-semibold text-slate-950 shadow-lg shadow-cyan-500/30 hover:scale-[1.02] transition cursor-pointer">Save product</button>
            </div>
        </form>
    </div>
</section>
